...
/**
 * This class has been edited so that it is thread safe and is synchronized.
 */
public class GameLogic {
	...
	/**
	* Gives the user a list of commands to use in game
	*/
	public void clientHelp(int playerID) {
		final Player player = this.players.get(playerID);

		player.sendMessage("<font color = green><br>How to play:<br>" +
				"<br>" +
				"You are the blue player on screen, the aim is to collect the amount of gold required by the map and make your way to the exit before other players.<br>" +
				"<br>" +
				"Enemy players are red<br>" +
				"<br>" +
				"Certain items such as as Lantern will help you along the way by providing certain benefits.<br>" +
				"Controls:<br>" +
				"Buttons<br>" +
				...
				"<br>Chat:<br>" +
				"<br>" +
				"The chat supports some color and font commands:<br>" +
				"/r - red<br>" +
				"/b - blue<br>" +
				"/g - green<br>" +
				"/o - orange<br>" +
	...
	public synchronized void clientMove(CompassDirection direction, int playerID)
		...
		this.players.get(playerID).apValue();
		advanceTurn(playerID);
		return;
	}
	 ...
	 */
	public synchronized void clientShout(String message) {
	...
	/**
	 * Sends a change to all players of the game.
	 *
	 */
	public synchronized void clientChange() {
		for (final Player player : this.players) {
			player.sendChange();
